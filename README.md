...
本周计划：
	...
	5. 目前存在一个问题，就是结构提是uint64存储的，但是在规则中使用了 = 进行判断，
	   但是在使用 = 时，ast类型是字符串类型，这样在进行比较时，会出现错误的情况。（在实际判断是否相等时，先转换数据类型）。
	6. 优先将事件获取以及事件匹配的多线程创建出来，优先搭建好架子，后续再处理上面的问题以及扩展功能（暂未想到解决办法）
	7. proc的缓存功能，以提供proc字段的提取功能（完成，由于是以线程读取的，有些执行了就消失的进程可能还没有缓存就已经从proc下删除了）
	8. 统一代码内的字段类型，如FIELD_TYPE_INT8等，用唯一的标识符来处理所有的字段类型
	9. 将ebpf的配置先弄好，支持所有系统调用的配置与读取，看能否匹配一下威胁库中的某些规则（完成）

现存问题：
	1. 当前进程缓存是由线程实现的，线程需要CPU调度，所以某些进程可能缓存不到，特别是运行时间及短的进程，这就导致在规则匹配时无法匹配对应的信息，导致规则匹配失败。
	2. 当前YAML解析不支持宏替换以及列表等操作
	3. 规则解析流程中还不支持 val() 替换，不支持规则字段跟参数
	4. 当前支持了部分判断逻辑，如 or and not > >= < <= = != contains icontains startswith endswith in 等
	5. 异步输出中，目前只支持了输出到文件
	6. 内核采集目前只支持 ebpf
	7. 事件采集、事件丰富以及规则匹配目前都是单线程的，效率不高，暂未实现多线程

...
int BPF_PROG(landlock_restrict_self_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_LANDLOCK_RESTRICT_SELF_E, -1);
...
int BPF_PROG(landlock_restrict_self_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_LANDLOCK_RESTRICT_SELF_X, ret);

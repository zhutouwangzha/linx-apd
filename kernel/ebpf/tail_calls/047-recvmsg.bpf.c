...
int BPF_PROG(recvmsg_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_RECVMSG_E, -1);
...
int BPF_PROG(recvmsg_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_RECVMSG_X, ret);
    ...
    uint64_t __msg = (uint64_t)get_pt_regs_argumnet(regs, 1);
    linx_ringbuf_store_u64(ringbuf, __msg);

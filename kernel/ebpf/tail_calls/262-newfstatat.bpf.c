...
int BPF_PROG(newfstatat_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_NEWFSTATAT_E, -1);
...
int BPF_PROG(newfstatat_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_NEWFSTATAT_X, ret);
    ...
    /* struct stat * statbuf */
    uint64_t __statbuf = (uint64_t)get_pt_regs_argumnet(regs, 2);
    linx_ringbuf_store_u64(ringbuf, __statbuf);

    /* int flag */
    int32_t __flag = (int32_t)get_pt_regs_argumnet(regs, 3);

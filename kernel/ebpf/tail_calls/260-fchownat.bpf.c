...
int BPF_PROG(fchownat_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_FCHOWNAT_E, -1);
...
int BPF_PROG(fchownat_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_FCHOWNAT_X, ret);
    ...
    /* uid_t user */
    uint32_t __user = (uint32_t)get_pt_regs_argumnet(regs, 2);
    linx_ringbuf_store_u32(ringbuf, __user);

    /* gid_t group */
    uint32_t __group = (uint32_t)get_pt_regs_argumnet(regs, 3);

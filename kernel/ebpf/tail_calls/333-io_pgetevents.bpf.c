...
int BPF_PROG(io_pgetevents_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_IO_PGETEVENTS_E, -1);
...
SEC("tp_btf/sys_exit")
int BPF_PROG(io_pgetevents_x, struct pt_regs *regs, long ret)
{
    linx_ringbuf_t *ringbuf = linx_ringbuf_get();
    if (!ringbuf) {
        return 0;
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_IO_PGETEVENTS_X, ret);
    ...
    uint64_t __usig = (uint64_t)get_pt_regs_argumnet(regs, 5);
    linx_ringbuf_store_u64(ringbuf, __usig);

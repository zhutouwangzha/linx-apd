...
int BPF_PROG(landlock_create_ruleset_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_LANDLOCK_CREATE_RULESET_E, -1);
...
int BPF_PROG(landlock_create_ruleset_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_LANDLOCK_CREATE_RULESET_X, ret);

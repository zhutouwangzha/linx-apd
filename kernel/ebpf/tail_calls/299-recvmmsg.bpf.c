...
int BPF_PROG(recvmmsg_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_RECVMMSG_E, -1);
...
int BPF_PROG(recvmmsg_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_RECVMMSG_X, ret);
    ...
    /* struct mmsghdr * mmsg */
    uint64_t __mmsg = (uint64_t)get_pt_regs_argumnet(regs, 1);
    linx_ringbuf_store_u64(ringbuf, __mmsg);

    /* unsigned int vlen */
    uint32_t __vlen = (uint32_t)get_pt_regs_argumnet(regs, 2);
    linx_ringbuf_store_u32(ringbuf, __vlen);

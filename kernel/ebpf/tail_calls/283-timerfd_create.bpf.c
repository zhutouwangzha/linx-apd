...
int BPF_PROG(timerfd_create_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_TIMERFD_CREATE_E, -1);
...
int BPF_PROG(timerfd_create_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_TIMERFD_CREATE_X, ret);

    /* int clockid */
    int32_t __clockid = (int32_t)get_pt_regs_argumnet(regs, 0);

...
int BPF_PROG(poll_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_POLL_E, -1);
...
int BPF_PROG(poll_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_POLL_X, ret);
    ...
    /* int timeout_msecs */
    int32_t __timeout_msecs = (int32_t)get_pt_regs_argumnet(regs, 2);
    linx_ringbuf_store_s32(ringbuf, __timeout_msecs);

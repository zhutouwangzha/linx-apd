...
int BPF_PROG(finit_module_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_FINIT_MODULE_E, -1);
...
int BPF_PROG(finit_module_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_FINIT_MODULE_X, ret);
    ...
    /* const char * uargs */
    uint64_t __uargs = (uint64_t)get_pt_regs_argumnet(regs, 1);

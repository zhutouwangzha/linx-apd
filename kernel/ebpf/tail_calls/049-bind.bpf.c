...
int BPF_PROG(bind_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_BIND_E, -1);
...
int BPF_PROG(bind_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_BIND_X, ret);
    ...
    uint64_t __umyaddr = (uint64_t)get_pt_regs_argumnet(regs, 1);
    linx_ringbuf_store_u64(ringbuf, __umyaddr);

...
int BPF_PROG(getsockopt_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_GETSOCKOPT_E, -1);
...
int BPF_PROG(getsockopt_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_GETSOCKOPT_X, ret);
    ...
    int32_t ___optlen = 0;
    if (__optlen) { 
        bpf_probe_read_user(&___optlen, sizeof(___optlen), __optlen);
    }
    linx_ringbuf_store_s32(ringbuf, ___optlen);

...
int BPF_PROG(mknod_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_MKNOD_E, -1);
...
int BPF_PROG(mknod_x, struct pt_regs *regs, long ret)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_MKNOD_X, ret);
    ...
    /* unsigned dev */
    uint32_t __dev = (uint32_t)get_pt_regs_argumnet(regs, 2);

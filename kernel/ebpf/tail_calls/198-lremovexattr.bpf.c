...
int BPF_PROG(lremovexattr_e, struct pt_regs *regs, long id)
{
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_LREMOVEXATTR_E, -1);
...
SEC("tp_btf/sys_exit")
int BPF_PROG(lremovexattr_x, struct pt_regs *regs, long ret)
{
    linx_ringbuf_t *ringbuf = linx_ringbuf_get();
    if (!ringbuf) {
        return 0;
    ...
    linx_ringbuf_load_event(ringbuf, LINX_EVENT_TYPE_LREMOVEXATTR_X, ret);

...
int BPF_PROG(dup3_x, struct pt_regs *regs, long ret)
{
    ...
    /* unsigned int oldfd */
    uint32_t __oldfd = (uint32_t)get_pt_regs_argumnet(regs, 0);
    linx_ringbuf_store_u32(ringbuf, __oldfd);

    /* unsigned int newfd */
    uint32_t __newfd = (uint32_t)get_pt_regs_argumnet(regs, 1);
    linx_ringbuf_store_u32(ringbuf, __newfd);

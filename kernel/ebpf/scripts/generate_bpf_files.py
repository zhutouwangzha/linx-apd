...
def generate_bpf_file(syscall_lower, enter_params):
    # 类型映射表：C类型 --> （存储函数，转换类型）
    type_map = {
        # int8_t
...
def process_header_file(input_path, output_dir):
    ...
    for line in header_content.splitlines():
        ...
        # 生成BPF文件
        bpf_content = generate_bpf_file(syscall_lower, enter_params)
        filename = os.path.join(output_dir, f"{syscall_lower}.bpf.c")
